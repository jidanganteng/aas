...
export default function AdminReportDetailPage() {
  ...
  // Status options untuk tampilan (sama seperti di dashboard)
  const statusOptions = [
    { value: 'pending',   label: 'MENUNGGU',  bg: '#FEF3C7', color: '#92400E', border: '#F59E0B' },
    { value: 'approved',  label: 'DISETUJUI', bg: '#D1FAE5', color: '#065F46', border: '#10B981' },
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([reportData, commentsData]) => {
        ...
        // Konversi status dari database (bisa uppercase PENDING/APPROVED/REJECTED atau bahasa Indonesia)
        if (reportObj.status) {
          const statusUpper = reportObj.status.toUpperCase();
          if (statusUpper === 'PENDING') reportObj.status = 'pending';
          else if (statusUpper === 'APPROVED') reportObj.status = 'approved';
          else if (statusUpper === 'REJECTED') reportObj.status = 'rejected';
          else if (reportObj.status === 'MENUNGGU') reportObj.status = 'pending';
          else if (reportObj.status === 'DISETUJUI') reportObj.status = 'approved';
          else if (reportObj.status === 'DITOLAK') reportObj.status = 'rejected';
        }
        setReport(reportObj);
  ...
  // Ambil badge status untuk ditampilkan
    ...
    <AuthGuard roles={['ADMIN', 'SUPER_ADMIN']}>
      <DashboardLayout>
        <style>{`
          ...
          .ad-status-badge { display:inline-block; padding:6px 12px; font-family:'DM Sans'; font-size:11px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; border:2px solid; border-radius:0; }
        ...
        <div className="ad-root">
          ...
          <div style={{ borderBottom: '2px solid #1A1A1A', paddingBottom: 14, marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            ...
            <div className="ad-status-badge" style={{ background: currentStatus.bg, color: currentStatus.color, borderColor: currentStatus.border }}>
              {currentStatus.label}
            </div>
          </div>
          ...
          <div className="ad-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16 }}>
            {/* KIRI: Detail Laporan (sama seperti sebelumnya) */}
            ...
            {/* KANAN: Komentar (sama seperti sebelumnya) */}
