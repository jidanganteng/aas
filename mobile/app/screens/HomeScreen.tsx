...
function StatCard({ label, value, color, icon }: {
  ...
  return (
    <Animated.View style={{ opacity: anim, transform: [{ scale: anim }] }}>
      <LinearGradient
        colors={[color + '22', color + '08'] as const}
          ...
          <View style={styles.rcFooter}>
            {item.user?.name ? (
              <View style={styles.rcUserRow}>
                <LinearGradient colors={[catColor, catColor + 'AA'] as const} style={styles.rcAvatar}>
        ...
        <View style={[styles.modalSheet, { paddingBottom: 0 }]}>
          ...
          <View style={styles.cmInputRow}>
            ...
            <TouchableOpacity onPress={sendComment} activeOpacity={0.8}>
              <LinearGradient colors={['#6366F1', '#818CF8'] as const} style={styles.cmSendGrad}>
...
// ─── Settings Modal ───────────────────────────────────────────────────────────
// Menerima `router` dan `onClose` agar bisa navigasi ke halaman lain
function SettingsModal({ visible, onClose, onLogout, router }: {
  visible: boolean;
  onClose: () => void;
  onLogout: () => void;
  ...
  const handleNavigate = (screen: string) => {
    onClose(); // tutup modal dulu sebelum navigasi
    // sedikit delay agar modal selesai close sebelum push
    setTimeout(() => {
  ...
  const items = [
    {
      ...
      label: 'Profil Saya',
      color: '#6366F1',
      onPress: () => handleNavigate('/screens/ProfileScreen'),
    },
    {
      icon: '🔔',
      label: 'Notifikasi',
      color: '#F59E0B',
      onPress: () => handleNavigate('/screens/NotificationScreen'),
    },
    {
      icon: '🔒',
      label: 'Ubah Password',
      color: '#10B981',
      onPress: () => handleNavigate('/screens/ProfileScreen'), // buka profil lalu tap ubah password
    },
    {
      icon: '📋',
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          {items.map((s, i) => (
            <TouchableOpacity key={i} style={styles.settingsItem} onPress={s.onPress} activeOpacity={0.7}>
...
export default function HomeScreen() {
  ...
  // Refresh userName setiap kali layar difokus (misal balik dari ProfileScreen)
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View style={[styles.headerInner, { opacity: headerAnim }]}>
          <View style={styles.headerTop}>
            ...
            {/* ── Tombol header kanan: Notifikasi + Pengaturan ── */}
            <View style={styles.headerBtnGroup}>
              {/* Tombol notifikasi langsung */}
              <TouchableOpacity
                style={styles.headerIconBtn}
                onPress={() => router.push('/screens/NotificationScreen' as any)}
                activeOpacity={0.8}
              >
                <Text style={styles.headerIconBtnText}>🔔</Text>
                {/* Badge notifikasi (opsional — hubungkan ke jumlah unread) */}
          ...
          <View style={styles.headerSummaryRow}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <LinearGradient colors={['#6366F1', '#818CF8'] as const} style={styles.profileShortcutAvatar}>
                <Text style={styles.profileShortcutInitial}>
                  {userName ? userName[0].toUpperCase() : 'P'}
                </Text>
              </LinearGradient>
              <Text style={styles.profileShortcutText}>Profil</Text>
            </TouchableOpacity>
          </View>
        </Animated.View>
      </LinearGradient>
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.sectionRow}>
              ...
              <TouchableOpacity
                style={styles.addBtn}
                onPress={() => router.push('/screens/AddReportScreen' as any)}
                activeOpacity={0.85}
              >
                <LinearGradient
      ...
      />
      {/* SettingsModal sekarang menerima `router` untuk navigasi */}
      <SettingsModal
...
const styles = StyleSheet.create({
  ...
  // Grup tombol kanan header (notifikasi + pengaturan)
  ...
  // Badge merah di atas ikon notifikasi
  notifBadge: {
  ...
  notifBadgeText: { color: '#fff', fontSize: 7, fontWeight: '900' },

  // Shortcut profil di summary row
  headerSummaryRow: { marginTop: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerSummaryPill: {
  ...
  profileShortcut: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  profileShortcutAvatar: {
    width: 28, height: 28, borderRadius: 14,
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 2, borderColor: 'rgba(255,255,255,0.3)',
  },
  profileShortcutInitial: { color: '#fff', fontSize: 12, fontWeight: '800' },
  profileShortcutText: { color: 'rgba(199,210,254,0.9)', fontSize: 12, fontWeight: '600' },

  // ── Ticker ──────────────────────────────────────────────────────────────────
  ticker: { backgroundColor: '#1E1B4B', height: 32, justifyContent: 'center', overflow: 'hidden' },
  tickerText: {
  ...
  statCard: {
    width: 128, borderRadius: 20, padding: 16,
  ...
  rcStatusPill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20 },
  ...
  rcIconBtn: { width: 34, height: 34, borderRadius: 10, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center' },
  ...
  cmCloseBtn: { width: 34, height: 34, borderRadius: 10, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center' },
  cmClose: { fontSize: 14, color: '#64748B', fontWeight: '700' },
  cmBubble: {
    backgroundColor: '#F8FAFC', borderRadius: 16, borderTopLeftRadius: 4,
    padding: 12, marginBottom: 8, maxWidth: '78%', alignSelf: 'flex-start',
  },
  cmBubbleMine: { backgroundColor: '#EEF2FF', alignSelf: 'flex-end', borderTopLeftRadius: 16, borderTopRightRadius: 4 },
  ...
  cmInput: {
    ...
    fontSize: 14, color: '#1E293B', maxHeight: 100, backgroundColor: '#F8FAFC',
  },
  ...
  settingsIconWrap: { width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  ...
  logoutBtn: { marginTop: 8, paddingVertical: 15, alignItems: 'center', borderRadius: 16, backgroundColor: '#FEF2F2' },
  logoutText: { color: '#EF4444', fontWeight: '800', fontSize: 15 },
  cancelBtn: { marginTop: 10, paddingVertical: 14, alignItems: 'center', backgroundColor: '#F8FAFC', borderRadius: 16 },
